...
import {
  IProductCategoriesData,
  ProductCategoriesQuery,
} from '../../app-features/categories/ProductCategoriesQueries'

export default function SubHeader(): JSX.Element {
  const { loading, error, data } = useQuery<IProductCategoriesData>(
    ProductCategoriesQuery
  )
  console.log(data)
  const rootProductCategories = data?.productCategories.filter(
    (cat) => cat.parent === null
  )
      ...
      <Flex maxW="1133px" w="100%">
        <Popover placement="bottom-start" autoFocus={false}>
          <PopoverTrigger>
            ...
            >
              <Icon as={MenuIcon} />
              <Text fontWeight="medium">Catalogul Produselor</Text>
            </Flex>
          </PopoverTrigger>
